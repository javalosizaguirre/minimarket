<?php
include_once RUTA_CLASES . 'marcaproducto.class.php';
class interfazMarcaProducto
{
    function principal()
    {
    ...
    function datagrid($criterio, $total_regs = 0, $pagina = 1, $nreg_x_pag = 50)
    {
        ...
        $Grid->accion(array(
            ...
            "xajax" => array(
                "fn" => "xajax__marcaproductoMantenimiento",
        ...
        $Grid->accion(array(
            ...
            "titulo" => "Editar Marca",
            "xajax" => array(
                "fn" => "xajax__interfazMarcaProductoEditar",
                "parametros" => array(
                    "flag" => "2",
                    "campos" => array("marca")
                )
        ...
        $Grid->paginador(array(
            "xajax" => "xajax__marcaproductoDatagrid",
    ...
    public function interfazEditar($marca)
    {
        $clase = new marcaproducto();
        $data = $clase->consultar('1', $marca);
        ...
        $html = '
            <form name="form" id="form" class="form-horizontal" onsubmit="return false" method="post">                
                    ...
                    <div class="form-group row">
                        <label class="col-sm-3 col-form-label" >Código:</label>
                        <div class="col-sm-9">
                            <input type="text" style ="width:30%" class="form-control"   id="txtCodigo" name="txtCodigo" value="' . $marca . '" maxlength="3" readonly />
                        </div>
                    </div>
                    <div class="form-group row">
                        <label class="col-sm-3 col-form-label" ><span style="color:red">(*)</span> Descripción:</label>
...
function _interfazMarcaProducto()
{
    $rpta = new xajaxResponse();
    $cls = new interfazMarcaProducto();
    $html = $cls->principal();
    $rpta->assign("container", "innerHTML", $html);
    $rpta->script("
    $('#btnNuevo').unbind('click').click(function() {
        xajax__interfazMarcaProductoNuevo();
    });");
    $rpta->script("
    $('#btnBuscar').unbind('click').click(function() {
    xajax__marcaproductoDatagrid(document.getElementById('txtBuscar').value);
    });");
    $rpta->script("
...
function _interfazMarcaProductoNuevo()
{
    ...
    $rpta->script("
        $('#btnGuardar').unbind('click').click(function() {
            xajax__marcaproductoMantenimiento('1',xajax.getFormValues('form'));
        });");
    return $rpta;
}

function _interfazMarcaProductoEditar($flag, $marca)
{
    $rpta = new xajaxResponse('UTF-8');
    $cls = new interfazMarcaProducto();
    ...
    $rpta->script("
        $('#btnEdiar').unbind('click').click(function() {
            xajax__marcaproductoMantenimiento('" . $flag . "',xajax.getFormValues('form'));
        });");
    return $rpta;
}

function _marcaproductoDatagrid($criterio, $total_regs = 0, $pagina = 1, $nregs = 50)
{
    $rpta = new xajaxResponse();
    $cls = new interfazMarcaProducto();
...
function _marcaproductoMantenimiento($flag, $form = '')
{
    $rpta = new xajaxResponse();
    $clase = new marcaproducto();
    $interfaz = new interfazMarcaProducto();
    if ($flag == '3') {
...
$xajax->register(XAJAX_FUNCTION, '_interfazMarcaProducto');
$xajax->register(XAJAX_FUNCTION, '_interfazMarcaProductoNuevo');
$xajax->register(XAJAX_FUNCTION, '_marcaproductoDatagrid');
$xajax->register(XAJAX_FUNCTION, '_marcaproductoMantenimiento');
$xajax->register(XAJAX_FUNCTION, '_interfazMarcaProductoEditar');
