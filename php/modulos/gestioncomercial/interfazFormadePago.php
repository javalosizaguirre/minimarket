<?php
include_once RUTA_CLASES . 'formapago.class.php';
class interfazFormaPago
{
    function principal()
    {
    ...
    function datagrid($criterio, $total_regs = 0, $pagina = 1, $nreg_x_pag = 50)
    {
        ...
        $Grid->accion(array(
            ...
            "xajax" => array(
                "fn" => "xajax__formapagoMantenimiento",
                "msn" => "¿Esta seguro de eliminar el registro?",
                "parametros" => array(
                    "flag" => "3",
                    "campos" => array("formapago")
                )
            )
        ));
        ...
        $Grid->accion(array(
            ...
            "xajax" => array(
                "fn" => "xajax__interfazFormaPagoEditar",
                "parametros" => array(
                    "flag" => "2",
                    "campos" => array("formapago")
                )
        ...
        $Grid->paginador(array(
            "xajax" => "xajax__formapagoDatagrid",
    ...
    public function interfazEditar($formapago)
    {
        $clase = new formapago();
        $data = $clase->consultar('1', $formapago);
        ...
        $html = '
            <form name="form" id="form" class="form-horizontal" onsubmit="return false" method="post">                
                    ...
                    <div class="form-group row">
                        <label class="col-sm-3 col-form-label" >Código:</label>
                        <div class="col-sm-9">
                            <input type="text" style ="width:30%" class="form-control"   id="txtCodigo" name="txtCodigo" value="' . $formapago . '" maxlength="3" readonly />
                        </div>
                    </div>
                    <div class="form-group row">
                        <label class="col-sm-3 col-form-label" ><span style="color:red">(*)</span> Descripción:</label>
...
function _interfazFormaPago()
{
    $rpta = new xajaxResponse();
    $cls = new interfazFormaPago();
    $html = $cls->principal();
    $rpta->assign("container", "innerHTML", $html);
    $rpta->script("
    $('#btnNuevo').unbind('click').click(function() {
        xajax__interfazFormaPagoNuevo();
    });");
    $rpta->script("
    $('#btnBuscar').unbind('click').click(function() {
    xajax__formapagoDatagrid(document.getElementById('txtBuscar').value);
    });");
    $rpta->script("
...
function _interfazFormaPagoNuevo()
{
    ...
    $rpta->script("
        $('#btnGuardar').unbind('click').click(function() {
            xajax__formapagoMantenimiento('1',xajax.getFormValues('form'));
        });");
    return $rpta;
}

function _interfazFormaPagoEditar($flag, $formapago)
{
    $rpta = new xajaxResponse('UTF-8');
    $cls = new interfazFormaPago();
    $html = $cls->interfazEditar($formapago);
    $rpta->script("$('#modal .modal-header h5').text('Registrar Perfil');");
    $rpta->assign("contenido", "innerHTML", $html[0]);
    $rpta->assign("footer", "innerHTML", $html[1]);
    $rpta->script("$('#modal').modal('show')");
    $rpta->script("
        $('#btnEdiar').unbind('click').click(function() {
            xajax__formapagoMantenimiento('" . $flag . "',xajax.getFormValues('form'));
        });");
    return $rpta;
}

function _formapagoDatagrid($criterio, $total_regs = 0, $pagina = 1, $nregs = 50)
{
    $rpta = new xajaxResponse();
    $cls = new interfazFormaPago();
...
function _formapagoMantenimiento($flag, $form = '')
{
    $rpta = new xajaxResponse();
    $clase = new formapago();
    $interfaz = new interfazFormaPago();
    if ($flag == '3') {
...
$xajax->register(XAJAX_FUNCTION, '_interfazFormaPago');
$xajax->register(XAJAX_FUNCTION, '_interfazFormaPagoNuevo');
$xajax->register(XAJAX_FUNCTION, '_formapagoDatagrid');
$xajax->register(XAJAX_FUNCTION, '_formapagoMantenimiento');
$xajax->register(XAJAX_FUNCTION, '_interfazFormaPagoEditar');
