<?php
include_once RUTA_CLASES . 'modeloproducto.class.php';
class interfazModeloProducto
{
    function principal()
    {
    ...
    function datagrid($criterio, $total_regs = 0, $pagina = 1, $nreg_x_pag = 50)
    {
        ...
        $Grid->accion(array(
            ...
            "xajax" => array(
                "fn" => "xajax__modeloproductoMantenimiento",
        ...
        $Grid->accion(array(
            ...
            "titulo" => "Editar Marca",
            "xajax" => array(
                "fn" => "xajax__interfazModeloProductoEditar",
                "parametros" => array(
                    "flag" => "2",
                    "campos" => array("modelo")
                )
        ...
        $Grid->paginador(array(
            "xajax" => "xajax__modeloproductoDatagrid",
    ...
    public function interfazEditar($modelo)
    {
        $clase = new modeloproducto();
        $data = $clase->consultar('1', $modelo);
        ...
        $html = '
            <form name="form" id="form" class="form-horizontal" onsubmit="return false" method="post">                
                    ...
                    <div class="form-group row">
                        <label class="col-sm-3 col-form-label" >Código:</label>
                        <div class="col-sm-9">
                            <input type="text" style ="width:30%" class="form-control"   id="txtCodigo" name="txtCodigo" value="' . $modelo . '" maxlength="3" readonly />
                        </div>
                    </div>
                    <div class="form-group row">
                        <label class="col-sm-3 col-form-label" ><span style="color:red">(*)</span> Descripción:</label>
...
function _interfazModeloProducto()
{
    $rpta = new xajaxResponse();
    $cls = new interfazModeloProducto();
    $html = $cls->principal();
    $rpta->assign("container", "innerHTML", $html);
    $rpta->script("
    $('#btnNuevo').unbind('click').click(function() {
        xajax__interfazModeloProductoNuevo();
    });");
    $rpta->script("
    $('#btnBuscar').unbind('click').click(function() {
    xajax__modeloproductoDatagrid(document.getElementById('txtBuscar').value);
    });");
    $rpta->script("
...
function _interfazModeloProductoNuevo()
{
    ...
    $rpta->script("
        $('#btnGuardar').unbind('click').click(function() {
            xajax__modeloproductoMantenimiento('1',xajax.getFormValues('form'));
        });");
    return $rpta;
}

function _interfazModeloProductoEditar($flag, $marca)
{
    $rpta = new xajaxResponse('UTF-8');
    $cls = new interfazModeloProducto();
    ...
    $rpta->script("
        $('#btnEdiar').unbind('click').click(function() {
            xajax__modeloproductoMantenimiento('" . $flag . "',xajax.getFormValues('form'));
        });");
    return $rpta;
}

function _modeloproductoDatagrid($criterio, $total_regs = 0, $pagina = 1, $nregs = 50)
{
    $rpta = new xajaxResponse();
    $cls = new interfazModeloProducto();
...
function _modeloproductoMantenimiento($flag, $form = '')
{
    $rpta = new xajaxResponse();
    $clase = new modeloproducto();
    $interfaz = new interfazModeloProducto();
    if ($flag == '3') {
...
$xajax->register(XAJAX_FUNCTION, '_interfazModeloProducto');
$xajax->register(XAJAX_FUNCTION, '_interfazModeloProductoNuevo');
$xajax->register(XAJAX_FUNCTION, '_modeloproductoDatagrid');
$xajax->register(XAJAX_FUNCTION, '_modeloproductoMantenimiento');
$xajax->register(XAJAX_FUNCTION, '_interfazModeloProductoEditar');
