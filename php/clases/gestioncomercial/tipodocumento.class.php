...
class tipodocumento extends connectdb
{
    function buscar($criterio, $buscar, $flagContar = 0, $paginaActual = 1, $regsPorPag = 20)
    {
        $data = array();
        $query = "CALL sp_tipodocumentoBuscar('$criterio', '$buscar', '$flagContar', '$paginaActual', '$regsPorPag')";
    ...
    function mantenedor($flag, $form)
    {
        ...
        $query = "CALL sp_tipodocumentoMantenedor('$flag', '$tipocomprobante', '$descripcion', '$activo', '$usuario')";
        $result = parent::query($query);
        if (!isset($result['error'])) {
            foreach ($result as $row) {
                $data[] = $row;
    ...
    {
        $data = array();
        $query = "CALL sp_tipodocumentoConsultar('$flag', '$criterio')";
        $result = parent::query($query);
        if (!isset($result['error'])) {
            foreach ($result as $row) {
                $data[] = $row;
