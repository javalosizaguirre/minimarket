...
class ventadetalle extends connectdb
{
    ...
    function mantenedor($flag, $detalle, $venta, $serie, $nrocomprobante, $producto, $cantidad, $precio)
    {
        $data = array();
        $usuario = $_SESSION["sys_usuario"];


        $query = "CALL sp_ventadetalleMantenedor('$flag', '$detalle', '$venta', '$serie', '$nrocomprobante', '$producto', '$cantidad', '$precio', '0.00', '$usuario')";
        $result = parent::query($query);
        if (!isset($result['error'])) {
            foreach ($result as $row) {
                $data[] = $row;
