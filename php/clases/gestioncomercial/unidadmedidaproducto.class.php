...
class unidadmedidaproducto extends connectdb
{
    function buscar($criterio, $buscar, $flagContar = 0, $paginaActual = 1, $regsPorPag = 20)
    {
        $data = array();
        $query = "CALL sp_unidadmedidaproductoBuscar('$criterio', '$buscar', '$flagContar', '$paginaActual', '$regsPorPag')";
        // echo "buscar--->   ".$query;
    ...
    function mantenedor($flag, $form)
    {
        ...
        $query = "CALL sp_unidadmedidaproductoMantenedor('$flag', '$perfil', '$descripcion', '$activo', '$usuario')";
        $result = parent::query($query);
        if (!isset($result['error'])) {
            foreach ($result as $row) {
                $data[] = $row;
    ...
    {
        $data = array();
        $query = "CALL sp_unidadmedidaproductoConsultar('$flag', '$criterio')";
        $result = parent::query($query);
        if (!isset($result['error'])) {
            foreach ($result as $row) {
                $data[] = $row;
